...
class CarpetBaggingRegressor():
    ...
    def get_oob_samples(self,X):
        '''This follows how the _generate_indices code works in sklearn.ensemble
           which is what's used by the BaggingRegressor class.
        '''
        n_samples   = X.shape[0]
        ...
        for i in range(self.n_estimators):
            inds = rand_state.randint(0,n_samples,n_samples)
    ...
    def one_round_of_prune_and_train(self, num_trees, y_pl, x_pl, num_to_prune):
        ...
        nx_pl, ny_pl, keep_inds = self.prune_my_data(x_pl, y_pl, num_to_prune, 0, tab_chg[:, 0].astype(int))
        # Retrain the random forest on pruned data
        self.model.fit(nx_pl, ny_pl)
        # Compute error metrics
        mae, ssqe, mse, rmse = self.get_stats_full_model(x_pl, y_pl)

        return mae, ssqe, mse, rmse, nx_pl, ny_pl, keep_inds
    ...
    def up_selection(self, X, y, xall, yall, num_grows, growth_size, grw_slctr):
        ...
        print("rmse of a model trained on all the data:", rmse)
        print("The size fo the full dataset is:",np.shape(yall))

        self.construct_model() # The model is reset becasue fo the above fit - see above comment
        self.model.fit(X, y)
        #error tracking
        mae, ssqe, mse, rmse = self.get_stats_full_model(xall, yall)
        print("rmse of all data from a model trained on the intial small dataset:", rmse)
        print("The size of the intial small dataset is:",np.shape(y))

        err_counter[2,0]= rmse
        err_counter[2,1]= len(y)
        
        idx_in,idx_out=self.removed_data_indexs(yall, y) # intilize index_arrays
    ...
    def down_selection(self, X, y, prune_itt=10, prune_amt=100):
        ...
        print("rmse of all data for a model trained on all the data:", rmse)
        ...
        for i in range(prune_itt):
            mae, ssqe, mse, rmse, x_new, y_new, keep_inds = self.one_round_of_prune_and_train(self.n_estimators, 
                                                                                              y_crnt, 
                                                                                              x_crnt, 
                                                                                              prune_amt)
            ...
            mae, ssqe, mse, rmse = self.get_stats_full_model(X, y)
            print("rmse of all data after {} round(s) of data down-selection:".format(i+1), rmse)
        ...
        return x_crnt, y_crnt, err_counter, keep_inds
